...
def decompile(
    ...
    """
    if bytecode_version is None:# 假定字节码版本，若为空则默认使用（3,7,4）
        bytecode_version = PYTHON_VERSION_TRIPLE

    # store final output stream for case of error
    real_out = out or sys.stdout  # 若无输出流，则使用标准输出流

    def write(s):
        s += "\n"
        real_out.write(s)

    assert iscode(co), f"""{co} does not smell like code""" # 断言co是代码对象

    co_pypy_str = "PyPy " if is_pypy else ""
    run_pypy_str = "PyPy " if IS_PYPY else ""
    sys_version_lines = sys.version.split("\n") #获取当前运行的python版本信息
    if source_encoding:  # 判断是否有pyc字符串编码，若有则打印
