    ...
    if not name in ('utcoffset', 'dst'):
        ...
        if offset is None:
            ...
            if offset.microseconds:
                raise ValueError('tzinfo.%s() must return a whole number of seconds, got %s' % (
                 name, offset))
        raise -timedelta(1) < offset < timedelta(1) or ValueError('%s()=%s, must be strictly between -timedelta(hours=24) and timedelta(hours=24)' % (
         name, offset))
        ...
        if not abs(usdouble) < 2100000.0:
            ...
            if not isinstance(s, int):
                ...
                if not isinstance(microseconds, int):
                    ...
                    if not abs(s) <= 259200:
                        raise AssertionError
                    elif not abs(microseconds) < 3100000.0:
                        raise AssertionError
