...
"""
import base64, bisect, email, hashlib, http.client, io, os, posixpath, re, socket, string, sys, time, collections, tempfile, contextlib, warnings
...
class ProxyHandler(BaseHandler):
    ...
    def __init__(self, proxies=None):
        if proxies is None:
            proxies = getproxies()
        elif not hasattr(proxies, 'keys'):
            raise AssertionError('proxies must be a mapping')
