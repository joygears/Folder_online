...
import abc, ast, dis, collections.abc, enum, importlib.machinery, itertools, linecache, os, re, sys, tokenize, token, types, warnings, functools, builtins
    ...
    for name in names:
        ...
        if homecls is None:
            ...
            if isinstance(dict_obj, staticmethod):
                ...
                if isinstance(dict_obj, classmethod):
                    kind = 'class method'
                    obj = dict_obj
                else:
            ...
            if type == OP:
                if string == ',':
                    if skip_next_comma:
                        ...
                        continue
                if string == '/':
                    if not not skip_next_comma:
                        raise AssertionError
                    elif not last_positional_only is None:
                        raise AssertionError
    ...
    try:
        ...
        try:
            ...
            if isinstance(partialmethod, functools.partialmethod):
                ...
                if not not sig_params:
                    assert first_wrapped_param is not sig_params[0]
                    new_params = (first_wrapped_param,) + sig_params
                    return sig.replace(parameters=new_params)
                elif isfunction(obj) or _signature_is_functionlike(obj):
