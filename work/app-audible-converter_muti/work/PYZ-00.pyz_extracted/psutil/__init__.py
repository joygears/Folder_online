...
# Embedded file name: psutil\__init__.py
...
_as_dict_attrnames = set([x for x in dir(Process) if not x.startswith('_') if x not in ('send_signal',
                                                                                        'suspend',
                                                                                        'resume',
                                                                                        'terminate',
                                                                                        'kill',
    ...
    for pid, proc in ls:
        try:
            if proc is None:
                ...
                if proc.is_running():
                    if attrs is not None:
                        proc.info = proc.as_dict(attrs=attrs,
                          ad_value=ad_value)
                    yield proc
                else:
                    yield add(pid)
...
def disk_partitions(all=False):
    ...
    if POSIX:
        new = []
        for item in ret:
            nt = item._replace(maxfile=(pathconf(item.mountpoint, 'PC_NAME_MAX')),
    ...
    for name, fam, addr, mask, broadcast, ptp in rawlist:
        if has_enums:
            try:
                ...
                if hasattr(_psplatform, 'AF_LINK'):
                    if _psplatform.AF_LINK == fam:
                        fam = _psplatform.AF_LINK
