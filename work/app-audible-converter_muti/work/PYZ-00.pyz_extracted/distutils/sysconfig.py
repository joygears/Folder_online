...
if os.name == 'nt':
    if project_base.lower().endswith(('\\pcbuild\\win32', '\\pcbuild\\amd64')):
...
if _sys_home and os.name == 'nt' and _sys_home.lower().endswith(('\\pcbuild\\win32',
                                                                 '\\pcbuild\\amd64')):
    ...
    if compiler.compiler_type == 'unix':
        if sys.platform == 'darwin':
            if not get_config_var('CUSTOMIZED_OSX_COMPILER'):
                ...
                cc, cxx, opt, cflags, ccshared, ldshared, shlib_suffix, ar, ar_flags = get_config_vars('CC', 'CXX', 'OPT', 'CFLAGS', 'CCSHARED', 'LDSHARED', 'SHLIB_SUFFIX', 'AR', 'ARFLAGS')
                if 'CC' in os.environ:
                    newcc = os.environ['CC']
                    if sys.platform == 'darwin':
...
def _init_posix():
    ...
    global _config_vars
    name = os.environ.get('_PYTHON_SYSCONFIGDATA_NAME', os.environ.get('_CONDA_PYTHON_SYSCONFIGDATA_NAME', '_sysconfigdata_{abi}_{platform}_{multiarch}'.format(abi=(sys.abiflags),
