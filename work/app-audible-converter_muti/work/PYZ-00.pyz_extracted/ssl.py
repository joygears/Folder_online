...
"""
import ipaddress, textwrap, re, sys, os
...
class SSLContext(_SSLContext):
    ...
    def wrap_socket(self, sock, server_side=False, do_handshake_on_connect=True, suppress_ragged_eofs=True, server_hostname=None, session=None):
        return SSLSocket(sock=sock, server_side=server_side, do_handshake_on_connect=do_handshake_on_connect,
    ...
    def wrap_bio(self, incoming, outgoing, server_side=False, server_hostname=None, session=None):
        sslobj = self._wrap_bio(incoming, outgoing, server_side=server_side, server_hostname=server_hostname)
    ...
    def __init__(self, sock=None, keyfile=None, certfile=None, server_side=False, cert_reqs=CERT_NONE, ssl_version=PROTOCOL_TLS, ca_certs=None, do_handshake_on_connect=True, family=AF_INET, type=SOCK_STREAM, proto=0, fileno=None, suppress_ragged_eofs=True, npn_protocols=None, ciphers=None, server_hostname=None, _context=None, _session=None):
        if _context:
            ...
            if not certfile:
                ...
                elif sock.getsockopt(SOL_SOCKET, SO_TYPE) != SOCK_STREAM:
            ...
            if sock is not None:
                socket.__init__(self, family=(sock.family),
                  type=(sock.type),
                  proto=(sock.proto),
                  fileno=(sock.fileno()))
        ...
        if connected:
            try:
                sslobj = self._context._wrap_socket(self, server_side, server_hostname)
                self._sslobj = SSLObject(sslobj, owner=self, session=(self._session))
...
def wrap_socket(sock, keyfile=None, certfile=None, server_side=False, cert_reqs=CERT_NONE, ssl_version=PROTOCOL_TLS, ca_certs=None, do_handshake_on_connect=True, suppress_ragged_eofs=True, ciphers=None):
    return SSLSocket(sock=sock, keyfile=keyfile, certfile=certfile, server_side=server_side,
      cert_reqs=cert_reqs,
