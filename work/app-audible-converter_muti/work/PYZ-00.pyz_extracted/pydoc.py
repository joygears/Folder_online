...
def visiblename(name, all=None, obj=None):
    ...
    if name in frozenset({'__spec__', '__loader__', '__date__', '__qualname__', '__slots__', '__version__', '__package__', '__builtins__', '__path__', '__credits__', '__module__', '__doc__', '__cached__', '__author__', '__file__', '__name__'}):
        return 0
    else:
...
class Doc:
    ...
    def getdocloc(self, object, basedir=os.path.join(sys.base_exec_prefix, 'lib', 'python%d.%d' % sys.version_info[:2])):
        ...
        if isinstance(object, type(os)):
            if object.__name__ in ('errno', 'exceptions', 'gc', 'imp', 'marshal', 'posix',
                                   'signal', 'sys', '_thread', 'zipimport') or file.startswith(basedir) and not file.startswith(os.path.join(basedir, 'site-packages')):
...
class HTMLDoc(Doc):
    ...
    def docmodule(self, object, name=None, mod=None, *ignored):
        ...
        for key, value in inspect.getmembers(object, inspect.isclass):
            if (all is not None or (inspect.getmodule(value) or object) is object) and visiblename(key, all, object):
        ...
        for key, value in inspect.getmembers(object, inspect.isroutine):
            if (all is not None or inspect.isbuiltin(value) or inspect.getmodule(value) is object) and visiblename(key, all, object):
...
def render_doc(thing, title='Python Library Documentation: %s', forceload=0, renderer=None):
    ...
    if renderer is None:
        ...
        object = inspect.ismodule(object) or inspect.isclass(object) or inspect.isroutine(object) or inspect.isgetsetdescriptor(object) or inspect.ismemberdescriptor(object) or isinstance(object, property) or type(object)
        desc += ' object'
