...
# Embedded file name: cryptography\hazmat\backends\openssl\backend.py
import collections, contextlib, itertools, typing, warnings
...
from cryptography.hazmat.backends.openssl.dh import _DHParameters, _DHPrivateKey, _DHPublicKey, _dh_params_dup
...
from cryptography.hazmat.bindings._rust import asn1, x509 as rust_x509
from cryptography.hazmat.bindings.openssl import binding
from cryptography.hazmat.primitives import hashes, serialization
from cryptography.hazmat.primitives.asymmetric import dh, dsa, ec, ed25519, ed448, rsa
from cryptography.hazmat.primitives.asymmetric.padding import MGF1, OAEP, PKCS1v15, PSS
from cryptography.hazmat.primitives.asymmetric.types import PRIVATE_KEY_TYPES
from cryptography.hazmat.primitives.ciphers.algorithms import AES, ARC4, Blowfish, CAST5, Camellia, ChaCha20, IDEA, SEED, SM4, TripleDES
...
class Backend(BackendInterface):
    ...
    def __init__(self):
        ...
        if self._fips_enabled:
            if self._lib.CRYPTOGRAPHY_NEEDS_OSRANDOM_ENGINE:
                warnings.warn("OpenSSL FIPS mode is enabled. Can't enable DRBG fork safety.", UserWarning)
        else:
            self.activate_osrandom_engine()
        self._dh_types = [
         self._lib.EVP_PKEY_DH]
        if self._lib.Cryptography_HAS_EVP_PKEY_DHX:
            self._dh_types.append(self._lib.EVP_PKEY_DHX)

    def __repr__(self):
        return '<OpenSSLBackend(version: {}, FIPS: {})>'.format(self.openssl_version_text(), self._fips_enabled)
    ...
    def osrandom_engine_implementation(self):
        ...
        with self._get_osurandom_engine() as (e):
            res = self._lib.ENGINE_ctrl_cmd(e, b'get_implementation', len(buf), buf, self._ffi.NULL, 0)
    ...
    def openssl_version_text(self):
        ...
        """
        return self._ffi.string(self._lib.OpenSSL_version(self._lib.OPENSSL_VERSION)).decode('ascii')
    ...
    def _evp_md_from_algorithm(self, algorithm):
        if algorithm.name == 'blake2b' or algorithm.name == 'blake2s':
            alg = '{}{}'.format(algorithm.name, algorithm.digest_size * 8).encode('ascii')
        else:
    ...
    def hash_supported(self, algorithm):
        if self._fips_enabled:
            if not isinstance(algorithm, self._fips_hashes):
                return False
        evp_md = self._evp_md_from_algorithm(algorithm)
    ...
    def cipher_supported(self, cipher, mode):
        if self._fips_enabled:
            if not isinstance(cipher, self._fips_ciphers) or isinstance(cipher, TripleDES) and not isinstance(mode, (CBC, ECB)):
                return False
        try:
    ...
    def register_cipher_adapter(self, cipher_cls, mode_cls, adapter):
        if (cipher_cls, mode_cls) in self._cipher_registry:
            raise ValueError('Duplicate registration for: {} {}.'.format(cipher_cls, mode_cls))
        self._cipher_registry[(cipher_cls, mode_cls)] = adapter

    def _register_default_ciphers(self):
        for mode_cls in [CBC, CTR, ECB, OFB, CFB, CFB8, GCM]:
            self.register_cipher_adapter(AES, mode_cls, GetCipherByName('{cipher.name}-{cipher.key_size}-{mode.name}'))
        ...
        for mode_cls in [CBC, CFB, OFB, ECB]:
            self.register_cipher_adapter(Blowfish, mode_cls, GetCipherByName('bf-{mode.name}'))

        for mode_cls in [CBC, CFB, OFB, ECB]:
            self.register_cipher_adapter(SEED, mode_cls, GetCipherByName('seed-{mode.name}'))
    ...
    def _register_x509_encoders(self):
        ...
        self._crl_extension_encode_handlers = _CRL_EXTENSION_ENCODE_HANDLERS.copy()
        self._crl_entry_extension_encode_handlers = _CRL_ENTRY_EXTENSION_ENCODE_HANDLERS.copy()
        self._ocsp_request_extension_encode_handlers = _OCSP_REQUEST_EXTENSION_ENCODE_HANDLERS.copy()
        self._ocsp_basicresp_extension_encode_handlers = _OCSP_BASICRESP_EXTENSION_ENCODE_HANDLERS.copy()
    ...
    def derive_pbkdf2_hmac(self, algorithm, length, salt, iterations, key_material):
        ...
        res = self._lib.PKCS5_PBKDF2_HMAC(key_material_ptr, len(key_material), salt, len(salt), iterations, evp_md, length, buf)
        self.openssl_assert(res == 1)
    ...
    def _evp_pkey_to_private_key(self, evp_pkey):
        ...
        if key_type == self._lib.EVP_PKEY_RSA:
            ...
            rsa_cdata = self._ffi.gc(rsa_cdata, self._lib.RSA_free)
            return _RSAPrivateKey(self, rsa_cdata, evp_pkey, self._rsa_skip_check_key)
        if key_type == self._lib.EVP_PKEY_DSA:
            dsa_cdata = self._lib.EVP_PKEY_get1_DSA(evp_pkey)
            self.openssl_assert(dsa_cdata != self._ffi.NULL)
            dsa_cdata = self._ffi.gc(dsa_cdata, self._lib.DSA_free)
            return _DSAPrivateKey(self, dsa_cdata, evp_pkey)
        if key_type == self._lib.EVP_PKEY_EC:
            ec_cdata = self._lib.EVP_PKEY_get1_EC_KEY(evp_pkey)
            self.openssl_assert(ec_cdata != self._ffi.NULL)
            ec_cdata = self._ffi.gc(ec_cdata, self._lib.EC_KEY_free)
            return _EllipticCurvePrivateKey(self, ec_cdata, evp_pkey)
        if key_type in self._dh_types:
            dh_cdata = self._lib.EVP_PKEY_get1_DH(evp_pkey)
            self.openssl_assert(dh_cdata != self._ffi.NULL)
            dh_cdata = self._ffi.gc(dh_cdata, self._lib.DH_free)
            return _DHPrivateKey(self, dh_cdata, evp_pkey)
        if key_type == getattr(self._lib, 'EVP_PKEY_ED25519', None):
            return _Ed25519PrivateKey(self, evp_pkey)
        if key_type == getattr(self._lib, 'EVP_PKEY_X448', None):
            return _X448PrivateKey(self, evp_pkey)
        if key_type == getattr(self._lib, 'EVP_PKEY_X25519', None):
            return _X25519PrivateKey(self, evp_pkey)
        if key_type == getattr(self._lib, 'EVP_PKEY_ED448', None):
        ...
        if isinstance(padding, PKCS1v15):
            ...
            if isinstance(padding, PSS):
                if isinstance(padding._mgf, MGF1):
                    if self._fips_enabled:
                        if isinstance(padding._mgf._algorithm, hashes.SHA1):
                            return True
                    return self.hash_supported(padding._mgf._algorithm)
            if isinstance(padding, OAEP):
                if isinstance(padding._mgf, MGF1):
                    return self._oaep_hash_supported(padding._mgf._algorithm) and self._oaep_hash_supported(padding._algorithm) and (padding._label is None or len(padding._label) == 0 or self._lib.Cryptography_HAS_RSA_OAEP_LABEL == 1)
            return False
    ...
    def generate_dsa_parameters(self, key_size):
        ...
        res = self._lib.DSA_generate_parameters_ex(ctx, key_size, self._ffi.NULL, 0, self._ffi.NULL, self._ffi.NULL, self._ffi.NULL)
        self.openssl_assert(res == 1)
            ...
            if algorithm is not None:
                ...
                if not isinstance(private_key, (
                 rsa.RSAPrivateKey, dsa.DSAPrivateKey, ec.EllipticCurvePrivateKey)):
                    raise TypeError('Key must be an rsa, dsa, ec, ed25519, or ed448 private key.')
                else:
                    raise isinstance(algorithm, hashes.HashAlgorithm) or TypeError('Algorithm must be a registered hash algorithm.')
        elif isinstance(algorithm, hashes.MD5):
            if not isinstance(private_key, rsa.RSAPrivateKey):
                raise ValueError('MD5 hash algorithm is only supported with RSA keys')

    def create_x509_csr(self, builder: x509.CertificateSigningRequestBuilder, private_key: PRIVATE_KEY_TYPES, algorithm: typing.Optional[hashes.HashAlgorithm]) -> x509.CertificateSigningRequest:
        ...
        self.openssl_assert(res == 1)
        res = self._lib.X509_REQ_set_subject_name(x509_req, _encode_name_gc(self, builder._subject_name))
        self.openssl_assert(res == 1)
        ...
        sk_extension = self._ffi.gc(sk_extension, lambda x: self._lib.sk_X509_EXTENSION_pop_free(x, self._ffi.addressof(self._lib._original_lib, 'X509_EXTENSION_free')))
        self._create_x509_extensions(extensions=(builder._extensions),
        ...
        for attr_oid, attr_val in builder._attributes:
            ...
            res = self._lib.X509_REQ_add1_attr_by_OBJ(x509_req, obj, x509.name._ASN1Type.UTF8String.value, attr_val, len(attr_val))
            self.openssl_assert(res == 1)
    ...
    def create_x509_certificate(self, builder: x509.CertificateBuilder, private_key: PRIVATE_KEY_TYPES, algorithm: typing.Optional[hashes.HashAlgorithm]) -> x509.Certificate:
        if not isinstance(builder, x509.CertificateBuilder):
            ...
            self.openssl_assert(res == 1)
            res = self._lib.X509_set_subject_name(x509_cert, _encode_name_gc(self, builder._subject_name))
            self.openssl_assert(res == 1)
            ...
            self.openssl_assert(res == 1)
            self._set_asn1_time(self._lib.X509_getm_notBefore(x509_cert), builder._not_valid_before)
            ...
            self._create_x509_extensions(extensions=(builder._extensions),
              ...
              gc=True)
            res = self._lib.X509_set_issuer_name(x509_cert, _encode_name_gc(self, builder._issuer_name))
            self.openssl_assert(res == 1)
    ...
    def create_x509_crl(self, builder: x509.CertificateRevocationListBuilder, private_key: PRIVATE_KEY_TYPES, algorithm: typing.Optional[hashes.HashAlgorithm]) -> x509.CertificateRevocationList:
        ...
        self.openssl_assert(res == 1)
        res = self._lib.X509_CRL_set_issuer_name(x509_crl, _encode_name_gc(self, builder._issuer_name))
        self.openssl_assert(res == 1)
        ...
        for revoked_cert in builder._revoked_certificates:
            ...
            serial_number = _encode_asn1_int_gc(self, revoked_cert.serial_number)
            res = self._lib.X509_REVOKED_set_serialNumber(x509_revoked, serial_number)
            self.openssl_assert(res == 1)
            ...
            self.openssl_assert(res == 1)
            self._create_x509_extensions(extensions=(revoked_cert.extensions),
    ...
    def _create_x509_extensions(self, extensions, handlers, x509_obj, add_func, gc):
        for i, extension in enumerate(extensions):
            x509_extension = self._create_x509_extension(handlers, extension)
            self.openssl_assert(x509_extension != self._ffi.NULL)
            if gc:
                x509_extension = self._ffi.gc(x509_extension, self._lib.X509_EXTENSION_free)
    ...
    def _create_x509_extension(self, handlers, extension):
        if isinstance(extension.value, x509.UnrecognizedExtension):
            ...
            if isinstance(extension.value, x509.TLSFeature):
                value = _encode_asn1_str_gc(self, asn1.encode_tls_feature(extension.value))
                return self._create_raw_x509_extension(extension, value)
            elif isinstance(extension.value, x509.PrecertPoison):
                value = _encode_asn1_str_gc(self, asn1.encode_precert_poison(extension.value))
                return self._create_raw_x509_extension(extension, value)
            else:
                if isinstance(extension.value, x509.PrecertificateSignedCertificateTimestamps):
                    value = _encode_asn1_str_gc(self, rust_x509.encode_precertificate_signed_certificate_timestamps(extension.value))
                    return self._create_raw_x509_extension(extension, value)
                try:
                    encode = handlers[extension.oid]
                except KeyError:
                    raise NotImplementedError('Extension not supported: {}'.format(extension.oid))

            ext_struct = encode(self, extension.value)
            nid = self._lib.OBJ_txt2nid(extension.oid.dotted_string.encode('ascii'))
            self.openssl_assert(nid != self._lib.NID_undef)
            return self._lib.X509V3_EXT_i2d(nid, 1 if extension.critical else 0, ext_struct)

    def create_x509_revoked_certificate(self, builder: x509.RevokedCertificateBuilder) -> x509.RevokedCertificate:
        ...
        return _RawRevokedCertificate(serial_number, revocation_date, x509.Extensions(builder._extensions))

    def load_pem_private_key(self, data, password):
        return self._load_key(self._lib.PEM_read_bio_PrivateKey, self._evp_pkey_to_private_key, data, password)

    def load_pem_public_key(self, data):
        mem_bio = self._bytes_to_bio(data)
        evp_pkey = self._lib.PEM_read_bio_PUBKEY(mem_bio.bio, self._ffi.NULL, self._ffi.NULL, self._ffi.NULL)
        ...
        self.openssl_assert(res == 1)
        rsa_cdata = self._lib.PEM_read_bio_RSAPublicKey(mem_bio.bio, self._ffi.NULL, self._ffi.NULL, self._ffi.NULL)
    ...
    def load_pem_parameters(self, data):
        mem_bio = self._bytes_to_bio(data)
        dh_cdata = self._lib.PEM_read_bio_DHparams(mem_bio.bio, self._ffi.NULL, self._ffi.NULL, self._ffi.NULL)
    ...
    def load_der_private_key(self, data, password):
        ...
        if key:
            ...
            return self._load_key(self._lib.d2i_PKCS8PrivateKey_bio, self._evp_pkey_to_private_key, data, password)
    ...
    def load_der_parameters(self, data):
        ...
        if self._lib.Cryptography_HAS_EVP_PKEY_DHX:
            ...
            res = self._lib.BIO_reset(mem_bio.bio)
            self.openssl_assert(res == 1)
            dh_cdata = self._lib.Cryptography_d2i_DHxparams_bio(mem_bio.bio, self._ffi.NULL)
            if dh_cdata != self._ffi.NULL:
                dh_cdata = self._ffi.gc(dh_cdata, self._lib.DH_free)
                return _DHParameters(self, dh_cdata)
        self._handle_key_loading_error()
    ...
    def _load_key(self, openssl_read_func, convert_func, data, password):
        ...
        evp_pkey = self._ffi.gc(evp_pkey, self._lib.EVP_PKEY_free)
        if password is not None:
            if userdata.called == 0:
                raise TypeError('Password was given but private key is not encrypted.')
        if not (password is not None and userdata.called == 1):
    ...
    def _handle_key_loading_error(self):
        ...
        if not errors:
            ...
            if errors[0]._lib_reason_match(self._lib.ERR_LIB_EVP, self._lib.EVP_R_BAD_DECRYPT) or errors[0]._lib_reason_match(self._lib.ERR_LIB_PKCS12, self._lib.PKCS12_R_PKCS12_CIPHERFINAL_ERROR) or self._lib.Cryptography_HAS_PROVIDERS and errors[0]._lib_reason_match(self._lib.ERR_LIB_PROV, self._lib.PROV_R_BAD_DECRYPT):
                raise ValueError('Bad decrypt. Incorrect password?')
            else:
                if any(error._lib_reason_match(self._lib.ERR_LIB_EVP, self._lib.EVP_R_UNSUPPORTED_PRIVATE_KEY_ALGORITHM) for error in errors):
                    raise ValueError('Unsupported public key algorithm.')
                else:
                    errors = binding._errors_with_text(errors)
                    raise ValueError('Could not deserialize key data. The data may be in an incorrect format or it may be encrypted with an unsupported algorithm.', errors)
    ...
    def generate_elliptic_curve_private_key(self, curve):
        ...
        if self.elliptic_curve_supported(curve):
            ...
            return _EllipticCurvePrivateKey(self, ec_cdata, evp_pkey)
        raise UnsupportedAlgorithm('Backend object does not support {}.'.format(curve.name), _Reasons.UNSUPPORTED_ELLIPTIC_CURVE)
    ...
    def load_elliptic_curve_private_numbers(self, numbers):
        ...
        self.openssl_assert(res == 1)
        ec_cdata = self._ec_key_set_public_key_affine_coordinates(ec_cdata, public.x, public.y)
        evp_pkey = self._ec_cdata_to_evp_pkey(ec_cdata)
        return _EllipticCurvePrivateKey(self, ec_cdata, evp_pkey)

    def load_elliptic_curve_public_numbers(self, numbers):
        ec_cdata = self._ec_key_new_by_curve(numbers.curve)
        ec_cdata = self._ec_key_set_public_key_affine_coordinates(ec_cdata, numbers.x, numbers.y)
        evp_pkey = self._ec_cdata_to_evp_pkey(ec_cdata)
        return _EllipticCurvePublicKey(self, ec_cdata, evp_pkey)
    ...
    def _create_ocsp_basic_response(self, builder, private_key, algorithm):
        ...
        evp_md = self._evp_md_non_null_from_algorithm(builder._response._algorithm)
        ...
        if builder._response._revocation_reason is None:
            ...
            reason = _CRL_ENTRY_REASON_ENUM_TO_CODE[builder._response._revocation_reason]
        if builder._response._revocation_time is None:
            rev_time = self._ffi.NULL
        else:
            rev_time = self._create_asn1_time_gc(builder._response._revocation_time)
        next_update = self._ffi.NULL
        if builder._response._next_update is not None:
            next_update = self._create_asn1_time_gc(builder._response._next_update)
        this_update = self._create_asn1_time_gc(builder._response._this_update)
        res = self._lib.OCSP_basic_add1_status(basic, certid, builder._response._cert_status.value, reason, rev_time, this_update, next_update)
        ...
        res = self._lib.OCSP_basic_sign(basic, ossl_cert, private_key._evp_pkey, evp_md, self._ffi.NULL, flags)
        if res != 1:
            errors = self._consume_errors_with_text()
            raise ValueError('Error while signing. responder_cert must be signed by private_key', errors)
        return basic

    def create_ocsp_response(self, response_status, builder, private_key, algorithm):
        if response_status is ocsp.OCSPResponseStatus.SUCCESSFUL:
            basic = self._create_ocsp_basic_response(builder, private_key, algorithm)
        else:
            basic = self._ffi.NULL
        ocsp_resp = self._lib.OCSP_response_create(response_status.value, basic)
    ...
    def elliptic_curve_exchange_algorithm_supported(self, algorithm, curve):
        if self._fips_enabled:
            if not isinstance(curve, self._fips_ecdh_curves):
                return False
    ...
    def _elliptic_curve_to_nid(self, curve):
        ...
        """
        curve_aliases = {'secp192r1':'prime192v1', 
         'secp256r1':'prime256v1'}
        curve_name = curve_aliases.get(curve.name, curve.name)
        curve_nid = self._lib.OBJ_sn2nid(curve_name.encode())
        if curve_nid == self._lib.NID_undef:
            raise UnsupportedAlgorithm('{} is not a supported elliptic curve'.format(curve.name), _Reasons.UNSUPPORTED_ELLIPTIC_CURVE)
        return curve_nid
    ...
    def _ec_key_determine_group_get_func(self, ctx):
        ...
        if nid == nid_two_field:
            if self._lib.Cryptography_HAS_EC2M:
    ...
    def _private_key_bytes(self, encoding, format, encryption_algorithm, key, evp_pkey, cdata):
            ...
            if not isinstance(format, serialization.PrivateFormat):
                ...
                if not isinstance(encryption_algorithm, serialization.KeySerializationEncryption):
                    ...
                    if isinstance(encryption_algorithm, serialization.NoEncryption):
                        ...
                        if isinstance(encryption_algorithm, serialization.BestAvailableEncryption):
    ...
    def _private_key_bytes_via_bio(self, write_bio, evp_pkey, password):
        ...
        return self._bio_func_output(write_bio, evp_pkey, evp_cipher, password, len(password), self._ffi.NULL, self._ffi.NULL)
    ...
    def generate_dh_parameters(self, generator, key_size):
        if key_size < dh._MIN_MODULUS_SIZE:
            raise ValueError('DH key_size must be at least {} bits'.format(dh._MIN_MODULUS_SIZE))
        ...
        res = self._lib.DH_generate_parameters_ex(dh_param_cdata, key_size, generator, self._ffi.NULL)
        self.openssl_assert(res == 1)
    ...
    def generate_dh_private_key_and_parameters(self, generator, key_size):
        return self.generate_dh_private_key(self.generate_dh_parameters(generator, key_size))
    ...
    def load_dh_private_numbers(self, numbers):
        ...
        self.openssl_assert(res == 1)
        if codes[0] != 0:
            if not (parameter_numbers.g == 2 and codes[0] ^ self._lib.DH_NOT_SUITABLE_GENERATOR == 0):
    ...
    def x509_name_bytes(self, name: Name) -> bytes:
        ...
        pp = self._ffi.gc(pp, lambda pointer: self._lib.OPENSSL_free(pointer[0]))
        self.openssl_assert(res > 0)
    ...
    def x25519_load_private_bytes(self, data):
        ...
        self.openssl_assert(evp_pkey != self._ffi.NULL)
        evp_pkey = self._ffi.gc(evp_pkey, self._lib.EVP_PKEY_free)
        self.openssl_assert(self._lib.EVP_PKEY_id(evp_pkey) == self._lib.EVP_PKEY_X25519)
        return _X25519PrivateKey(self, evp_pkey)
    ...
    def x448_load_public_bytes(self, data):
        ...
        evp_pkey = self._lib.EVP_PKEY_new_raw_public_key(self._lib.NID_X448, self._ffi.NULL, data, len(data))
        self.openssl_assert(evp_pkey != self._ffi.NULL)
        evp_pkey = self._ffi.gc(evp_pkey, self._lib.EVP_PKEY_free)
        return _X448PublicKey(self, evp_pkey)
    ...
    def x448_load_private_bytes(self, data):
        ...
        data_ptr = self._ffi.from_buffer(data)
        evp_pkey = self._lib.EVP_PKEY_new_raw_private_key(self._lib.NID_X448, self._ffi.NULL, data_ptr, len(data))
        self.openssl_assert(evp_pkey != self._ffi.NULL)
        evp_pkey = self._ffi.gc(evp_pkey, self._lib.EVP_PKEY_free)
        return _X448PrivateKey(self, evp_pkey)
    ...
    def ed25519_load_public_bytes(self, data):
        ...
        evp_pkey = self._lib.EVP_PKEY_new_raw_public_key(self._lib.NID_ED25519, self._ffi.NULL, data, len(data))
        self.openssl_assert(evp_pkey != self._ffi.NULL)
        evp_pkey = self._ffi.gc(evp_pkey, self._lib.EVP_PKEY_free)
        return _Ed25519PublicKey(self, evp_pkey)
    ...
    def ed25519_load_private_bytes(self, data):
        ...
        data_ptr = self._ffi.from_buffer(data)
        evp_pkey = self._lib.EVP_PKEY_new_raw_private_key(self._lib.NID_ED25519, self._ffi.NULL, data_ptr, len(data))
        self.openssl_assert(evp_pkey != self._ffi.NULL)
        evp_pkey = self._ffi.gc(evp_pkey, self._lib.EVP_PKEY_free)
        return _Ed25519PrivateKey(self, evp_pkey)
    ...
    def ed448_load_public_bytes(self, data):
        ...
        evp_pkey = self._lib.EVP_PKEY_new_raw_public_key(self._lib.NID_ED448, self._ffi.NULL, data, len(data))
        self.openssl_assert(evp_pkey != self._ffi.NULL)
        evp_pkey = self._ffi.gc(evp_pkey, self._lib.EVP_PKEY_free)
        return _Ed448PublicKey(self, evp_pkey)
    ...
    def ed448_load_private_bytes(self, data):
        ...
        data_ptr = self._ffi.from_buffer(data)
        evp_pkey = self._lib.EVP_PKEY_new_raw_private_key(self._lib.NID_ED448, self._ffi.NULL, data_ptr, len(data))
        self.openssl_assert(evp_pkey != self._ffi.NULL)
        evp_pkey = self._ffi.gc(evp_pkey, self._lib.EVP_PKEY_free)
        return _Ed448PrivateKey(self, evp_pkey)
    ...
    def derive_scrypt(self, key_material, salt, length, n, r, p):
        buf = self._ffi.new('unsigned char[]', length)
        key_material_ptr = self._ffi.from_buffer(key_material)
        res = self._lib.EVP_PBE_scrypt(key_material_ptr, len(key_material), salt, len(salt), n, r, p, scrypt._MEM_LIMIT, buf, length)
        if res != 1:
            errors = self._consume_errors_with_text()
            min_memory = 128 * n * r // 1048576
            raise MemoryError('Not enough memory to derive key. These parameters require {} MB of memory.'.format(min_memory), errors)
        return self._ffi.buffer(buf)[:]

    def aead_cipher_supported(self, cipher):
        cipher_name = aead._aead_cipher_name(cipher)
        if self._fips_enabled:
            if cipher_name not in self._fips_aead:
                return False
    ...
    def load_key_and_certificates_from_pkcs12(self, data, password):
        ...
        return (
         key, cert, additional_certificates)
    ...
    def serialize_key_and_certificates_to_pkcs12(self, name, key, cert, cas, encryption_algorithm):
        ...
        with self._zeroed_null_terminated_buf(password) as (password_buf):
            with self._zeroed_null_terminated_buf(name) as (name_buf):
                ...
                p12 = self._lib.PKCS12_create(password_buf, name_buf, evp_pkey, ossl_cert, sk_x509, nid_key, nid_cert, pkcs12_iter, mac_iter, 0)
    ...
    def pkcs7_sign(self, builder, encoding, options):
        ...
        p7 = self._lib.PKCS7_sign(self._ffi.NULL, self._ffi.NULL, certs, self._ffi.NULL, init_flags)
        ...
        if pkcs7.PKCS7Options.NoCapabilities in options:
            ...
            if pkcs7.PKCS7Options.NoAttributes in options:
            ...
            for certificate, private_key, hash_algorithm in builder._signers:
                ...
                p7signerinfo = self._lib.PKCS7_sign_add_signer(p7, ossl_cert, private_key._evp_pkey, md, signer_flags)
            ...
            if encoding is serialization.Encoding.SMIME:
                ...
                if encoding is serialization.Encoding.PEM:
                    ...
                    self.openssl_assert(res == 1)
                    res = self._lib.PEM_write_bio_PKCS7_stream(bio_out, p7, bio.bio, final_flags)
                else:
