...
def _enc_dec_rsa_pkey_ctx(backend, key: typing.Union[('_RSAPrivateKey', '_RSAPublicKey')], data: bytes, padding_enum: int, padding: AsymmetricPadding) -> bytes:
...
def _rsa_sig_setup(backend, padding, algorithm, key, init_func):
    ...
    if isinstance(padding, PSS):
        res = backend._lib.EVP_PKEY_CTX_set_rsa_pss_saltlen(pkey_ctx, _get_rsa_pss_salt_length(padding, key, algorithm))
        backend.openssl_assert(res > 0)
        mgf1_md = backend._evp_md_non_null_from_algorithm(padding._mgf._algorithm)
        res = backend._lib.EVP_PKEY_CTX_set_rsa_mgf1_md(pkey_ctx, mgf1_md)
        backend.openssl_assert(res > 0)
