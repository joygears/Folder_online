...
# Embedded file name: cryptography\hazmat\backends\openssl\dsa.py
import typing
...
from cryptography.hazmat.primitives.asymmetric import AsymmetricSignatureContext, AsymmetricVerificationContext, dsa, utils as asym_utils
...
def _dsa_sig_sign(backend, private_key, data):
    ...
    res = backend._lib.DSA_sign(0, data, len(data), sig_buf, buflen, private_key._dsa_cdata)
...
def _dsa_sig_verify(backend, public_key, signature, data):
    res = backend._lib.DSA_verify(0, data, len(data), signature, len(signature), public_key._dsa_cdata)
    if res != 1:
...
class _DSAVerificationContext(AsymmetricVerificationContext):
    ...
    def verify(self) -> None:
        ...
        _dsa_sig_verify(self._backend, self._public_key, self._signature, data_to_verify)


class _DSASignatureContext(AsymmetricSignatureContext):

    def __init__(self, backend, private_key: dsa.DSAPrivateKey, algorithm: hashes.HashAlgorithm):
        self._backend = backend
...
class _DSAPrivateKey(dsa.DSAPrivateKey):
    ...
    def __init__(self, backend, dsa_cdata, evp_pkey):
        ...
        p = self._backend._ffi.new('BIGNUM **')
        self._backend._lib.DSA_get0_pqg(dsa_cdata, p, self._backend._ffi.NULL, self._backend._ffi.NULL)
    ...
    def private_numbers(self) -> dsa.DSAPrivateNumbers:
        ...
        return dsa.DSAPrivateNumbers(public_numbers=dsa.DSAPublicNumbers(parameter_numbers=dsa.DSAParameterNumbers(p=(self._backend._bn_to_int(p[0])),
          q=(self._backend._bn_to_int(q[0])),
          g=(self._backend._bn_to_int(g[0]))),
    ...
    def public_key(self) -> dsa.DSAPublicKey:
        ...
        pub_key = self._backend._ffi.new('BIGNUM **')
        self._backend._lib.DSA_get0_key(self._dsa_cdata, pub_key, self._backend._ffi.NULL)
        self._backend.openssl_assert(pub_key[0] != self._backend._ffi.NULL)
        pub_key_dup = self._backend._lib.BN_dup(pub_key[0])
        res = self._backend._lib.DSA_set0_key(dsa_cdata, pub_key_dup, self._backend._ffi.NULL)
    ...
    def private_bytes(self, encoding: serialization.Encoding, format: serialization.PrivateFormat, encryption_algorithm: serialization.KeySerializationEncryption) -> bytes:
        return self._backend._private_key_bytes(encoding, format, encryption_algorithm, self, self._evp_pkey, self._dsa_cdata)
...
class _DSAPublicKey(dsa.DSAPublicKey):
    ...
    def verifier(self, signature: bytes, signature_algorithm: hashes.HashAlgorithm) -> AsymmetricVerificationContext:
        ...
        _check_not_prehashed(signature_algorithm)
        return _DSAVerificationContext(self._backend, self, signature, signature_algorithm)
    ...
    def public_numbers(self) -> dsa.DSAPublicNumbers:
        ...
        self._backend._lib.DSA_get0_key(self._dsa_cdata, pub_key, self._backend._ffi.NULL)
        self._backend.openssl_assert(pub_key[0] != self._backend._ffi.NULL)
        return dsa.DSAPublicNumbers(parameter_numbers=dsa.DSAParameterNumbers(p=(self._backend._bn_to_int(p[0])),
          q=(self._backend._bn_to_int(q[0])),
          g=(self._backend._bn_to_int(g[0]))),
          y=(self._backend._bn_to_int(pub_key[0])))
    ...
    def verify(self, signature: bytes, data: bytes, algorithm: typing.Union[(asym_utils.Prehashed, hashes.HashAlgorithm)]) -> None:
        data, algorithm = _calculate_digest_and_algorithm(self._backend, data, algorithm)
