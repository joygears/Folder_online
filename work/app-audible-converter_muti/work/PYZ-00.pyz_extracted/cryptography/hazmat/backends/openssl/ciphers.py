...
class _CipherContext(object):
    ...
    def __init__(self, backend, cipher, mode, operation):
        ...
        if res == 0:
            if lib.CRYPTOGRAPHY_OPENSSL_111D_OR_GREATER and errors[0]._lib_reason_match(lib.ERR_LIB_EVP, lib.EVP_R_XTS_DUPLICATED_KEYS) or lib.Cryptography_HAS_PROVIDERS and errors[0]._lib_reason_match(lib.ERR_LIB_PROV, lib.PROV_R_XTS_DUPLICATED_KEYS):
            ...
            if res == 0:
                ...
                if not errors:
                    if isinstance(self._mode, modes.GCM):
                        raise InvalidTag
                self._backend.openssl_assert((errors[0]._lib_reason_match(self._backend._lib.ERR_LIB_EVP, self._backend._lib.EVP_R_DATA_NOT_MULTIPLE_OF_BLOCK_LENGTH) or self._backend._lib.Cryptography_HAS_PROVIDERS and errors[0]._lib_reason_match(self._backend._lib.ERR_LIB_PROV, self._backend._lib.PROV_R_WRONG_FINAL_BLOCK_LENGTH)),
                  errors=errors)
    ...
    def finalize_with_tag(self, tag: bytes) -> bytes:
        ...
        if tag_len < self._mode._min_tag_length:
            ...
            if tag_len > self._block_size_bytes:
