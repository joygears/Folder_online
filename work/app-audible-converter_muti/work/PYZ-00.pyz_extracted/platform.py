...
def linux_distribution(distname='', version='', id='', supported_dists=_supported_dists, full_distribution_name=1):
    import warnings
    warnings.warn('dist() and linux_distribution() functions are deprecated in Python 3.5', PendingDeprecationWarning,
      stacklevel=2)
...
_sys_version_parser = re.compile('([\\w.+]+)\\s*(?:\\|[^|]*\\|)?\\s*\\(#?([^,]+)(?:,\\s*([\\w ]*)(?:,\\s*([\\w :]*))?)?\\)\\s*\\[([^\\]]+)\\]?', re.ASCII)
...
def platform(aliased=0, terse=0):
    ...
    if system == 'Windows':
        ...
        with warnings.catch_warnings():
            warnings.filterwarnings('ignore', 'dist\\(\\) and linux_distribution\\(\\) functions are deprecated .*', PendingDeprecationWarning)
