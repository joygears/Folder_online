    ...
    if not value:
        ...
        if value[0] in CFWS_LEADER:
            ...
            if value:
                ...
                digits.isdigit() or mime_version.defects.append(errors.InvalidHeaderDefect('Expected MIME minor version number but found {!r}'.format(digits)))
                mime_version.append(ValueTerminal(digits, 'xtext'))
            else:
        ...
        if token is not None:
            ...
            if value:
                ...
                if remainder is not None:
                    ...
                    token, value = get_value(value)
            appendto.append(token)
            assert remainder is not None and not value, value
            value = remainder
        return (
         param, value)
            ...
            if not part.as_ew_allowed:
                ...
                if part.syntactic_break:
                    ...
                    if policy.linesep not in encoded_part:
                        ...
                        continue
            parts = hasattr(part, 'encode') or list(part) + parts
        else:
        ...
        if part.syntactic_break:
            if len(tstr) + 1 <= maxlen:
                ...
                if newline or part.startswith_fws():
                    ...
                    continue
        newparts = hasattr(part, 'encode') or list(part)
        if not part.as_ew_allowed:
            wrap_as_ew_blocked += 1
