...
Process::id_type Process::open(const string_type &command, const string_type &path) noexcept {
  ...
  if (stdout_wr_p)
  startup_info.hStdError = stdout_wr_p;
  ...
  process_command+="\"";
#endif
  wstring wprocess_command = L"";
  Utf8_To_Unicode(process_command, wprocess_command);
  BOOL bSuccess = CreateProcessW(nullptr, (wchar_t *)wprocess_command.c_str(), nullptr, nullptr, TRUE, CREATE_NO_WINDOW,
                                nullptr,nullptr, &startup_info, &process_info);

  if(!bSuccess) {
    logPrint("win CreateProcess fails, GetLastError code :" << GetLastError() << "  Execute the command as " << process_command);
  ...
  if(stdout_fd) {
    stdout_thread=std::thread([this](){
      ...
      for (;;) {
        ...
        if(!bSuccess || n == 0)
          break;
        stdoutFilter(buffer.get(), static_cast<size_t>(n));
      }
    });
  }
  if(stderr_fd) {
