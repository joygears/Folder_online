...
def getLicense_b64():
    with open("manifest.json","r") as fp:
        reuslt = fp.read()
    data = json.loads(reuslt)
    return data["result"]["video_tracks"][0]['license']['licenseResponseBase64']
class WvDownloader(object):
    ...
    def do_decrypt(self, config):
        ...
        license_b64 = self.client.get_license(chal, sess_id)
        #license_b64 = ""
        if not license_b64:
            return False
    ...
    def download_and_convert_subtitle(self, subtitle,save_path):
        language_code = subtitle.language_code


        output_filename = os.path.join(save_path, '{filename}_subtitles_{language_code}.{subtitle_type}').format(filename=self.filename, language_code=language_code,subtitle_type=self.config.subtitle_format)
