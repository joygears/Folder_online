...
void MyContentDecryptionModuleProxy::CreateSessionAndGenerateRequest(uint32_t promise_id, int session_type, int init_data_type, const uint8_t* init_data, uint32_t init_data_size)
{
    ...
    m_host->OnResolveNewSessionPromise(promise_id, sessionId.c_str(), sessionId.size());
...
{
    Keys_info key_infos;
    char key_id[] = {0x00,0x00,0x00,0x00, 0x06,0xfe,0x81,0x09 ,0x00,0x00,0x00,0x00 ,0x00,0x00,0x00,0x00 };
    key_infos.key_id = key_id;
    key_infos.size = 16;
    key_infos.status = 0;
    key_infos.system_code = 0;

    m_host->SetTimer(1000, (void *)1);
    m_host->OnSessionKeysChange(session_id, session_id_size, 1, &key_infos, 1);
    if (!m_instance)
    {
        Log("instance is null, %d", 96);
        return;
