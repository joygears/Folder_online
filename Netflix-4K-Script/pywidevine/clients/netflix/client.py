...
class NetflixClient(object):
    ...
    def __perform_key_handshake(self):
        ...
        payload['payload'] = base64.b64encode(json.dumps({
            'sequencenumber': 1,
            'messageid': self.current_message_id,
            'endofmsg': True,
            'data': ''
        ...
        request = json.dumps(request) + json.dumps(payload)

        self.logger.debug('Key Handshake Request:')
        self.logger.debug(json.dumps(request))

        resp = self.session.post(nf_cfg.MANIFEST_ENDPOINT, request, headers={'User-Agent': 'Gibbon/2018.1.6.3/2018.1.6.3: Netflix/2018.1.6.3 (DEVTYPE=NFANDROID2-PRV-FIRETVSTICK2016; CERTVER=0)'}, proxies=self.client_config.get_proxies())
